    ...
    private fun handleUdp(pkt: Pkt) {
        ...
        val relay = udpRelays.getOrPut(key) {
            UdpRelay(
                ...
                onRx = { n -> totalRx.addAndGet(n.toLong()) }
            )
        }
        relay.send(payload)
    }
    ...
    private class UdpRelay(
        ...
        init {
            try {
                ...
                protectSocket(ds)
                ds.connect(InetAddress.getByAddress(remoteIp), remotePort)
                datagramSocket = ds
            } catch (e: Exception) {
        ...
        fun send(data: ByteArray) {
            try {
                datagramSocket?.let { ds ->
                    val dp = DatagramPacket(data, data.size, InetAddress.getByAddress(remoteIp), remotePort)
                    ds.send(dp)
                }
            } catch (e: Exception) {
            }
