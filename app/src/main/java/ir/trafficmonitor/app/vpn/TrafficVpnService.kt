...
class TrafficVpnService : VpnService() {
    ...
    private fun Pkt(key: String): String = "${ipStr(srcIp)}:$srcPort-${ipStr(dstIp)}:$dstPort"
    ...
    private fun handleTcp(pkt: Pkt) {
        ...
        if (syn && !ack) {
            ...
            flow.seqRx++
            LogBuffer.append("NEW TCP ${ipStr(pkt.dstIp)}:${pkt.dstPort} -> ${ipStr(pkt.srcIp)}:${pkt.srcPort}")
            return
        }
    ...
    private fun ipStr(b: ByteArray): String = try { InetAddress.getByAddress(b).hostAddress ?: "?" } catch (e: Exception) { "?" }
